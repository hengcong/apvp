    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for step in range(gradient_steps):
            ...
            for (current_action, next_action, current_q_behavior, current_q_novice, action_similarity) in zip(replay_data.actions_novice, next_actions, current_q_behavior_values, current_q_novice_values, action_similaritys):
                ...
                #min_action_similarity = action_similarity.min().item()
                #max_action_similarity = action_similarity.max().item()
                #stat_recorder["action_similarity_min"].append(min_action_similarity)
                #stat_recorder["action_similarity_max"].append(max_action_similarity)
                ...
                l += th.mean(replay_data.interventions * self.cql_coefficient *(F.mse_loss(current_q_novice, -F.mse_loss(replay_data.actions_novice, replay_data.actions_behavior))))
                    ...
                    # l += self.cql_coefficient *(F.mse_loss(current_q_behavior, 0.1 * action_similarity))
                    # l += self.cql_coefficient *(F.mse_loss(current_q_novice, -0.1 * action_similarity))
                # l += th.mean(
                ...
                l += th.mean(
                    #replay_data.interventions * 
                    self.cql_coefficient *
                    (F.mse_loss(current_q_novice, -0.1 * action_similaritys))
                )    
