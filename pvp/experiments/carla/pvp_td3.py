...
class ExpertNetwork(nn.Module):
    def __init__(self, feature_extractor, action_space):
        super(ExpertNetwork, self).__init__()
        self.feature_extractor = feature_extractor
        self.policy = nn.Sequential(
            nn.Linear(256, 256),  # Ensure the input size matches the output size of the feature extractor
            nn.ReLU(),
...
class PVPTD3(TD3):
    def __init__(self, use_balance_sample=True, q_value_bound=1., *args, **kwargs):
        ...
        self.shared_feature_extractor = OurFeaturesExtractor(self.observation_space, features_dim=256 + other_feat_dim).to(self.device)
        self.expert_network = ExpertNetwork(self.shared_feature_extractor, self.action_space).to(self.device)
        self.expert_optimizer = th.optim.Adam(self.expert_network.parameters(), lr=1e-3)
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for step in range(gradient_steps):
            ...
            if self._n_updates % self.policy_delay == 0:
                ...
                # Compare expert and agent policies using KL divergence
                expert_actions = self.expert_network(replay_data.observations)
                agent_actions = self.actor(replay_data.observations)

                # Assuming actions follow Gaussian distribution
                agent_log_prob = th.distributions.Normal(agent_actions, 1).log_prob(agent_actions)
                expert_log_prob = th.distributions.Normal(expert_actions, 1).log_prob(expert_actions)

                kl_divergence = F.kl_div(agent_log_prob, expert_log_prob, reduction='batchmean')

                # Optimize the actor with KL divergence
                actor_loss += kl_divergence
                ...
                self.logger.record("train/policy_kl_divergence", kl_divergence.item())
    ...
    def train_expert_network(self, batch_size: int = 100) -> None:
        if self.human_data_buffer.pos > batch_size:
            ...
            expert_loss = F.mse_loss(
                self.expert_network(replay_data_human.observations), replay_data_human.actions_behavior
            )
